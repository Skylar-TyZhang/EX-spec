...
app_ui = ui.page_fluid(
    ...
    ui.div(
        ...
        ui.div(
            ...
            ui.p(
                "Interactive tool for exploring satellite sensor spectral data",
                ...
                ui.div(
                    ...
                    ui.div(
                        ...
                        ui.div(
                            ...
                            ui.div(
                                ui.input_action_button(
                                    "download_spectra_table",
                                    "Download Data",
                                    class_="btn-success download-btn btn-sm",
                                ),
                                ui.input_action_button(
                                    "download_spectra_plot",
                                    "Download Plot",
                                    class_="btn-info download-btn btn-sm",
                                ),
                                style="float: right;",
...
def server(input, output, session):
    ...
    def status_info():
        ...
        if not current_lib():
            return ui.div(
                ui.p(
                    "⚠️ No spectral library loaded. Please check the data path and try loading data.",
        ...
        return ui.div(
            {"class": "status-text"},
            ui.p(
                f"📡 {input.satellite()} | 📊 {num_spectra} spectra | 🎛️ {num_bands} bands | 🔬 {input.mineral_family() or 'None'} | 📈 {len(get_filtered_minerals())} samples",
                style="margin: 5px 0;",
            ),
        )
    ...
    @reactive.event(
        ...
        input.show_band_centers,
        input.show_band_ranges,
        input.show_response_functions,
    )
    ...
    def main_plot():
        ...
        if not current_lib() or not input.mineral_family():
            ...
            ax.text(
                0.5,
                0.5,
                "Select a mineral family to view spectral data",
                ha="center",
                va="center",
    ...
    def download_spectra_table():
        ...
        try:
            ...
            download_message.set(
                f"✅ Spectral data exported as {filename} ({len(df)} records)"
            )
